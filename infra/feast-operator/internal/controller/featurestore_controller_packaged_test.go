...
var _ = Describe("Packaged feature repository validation", func() {
	ctx := context.Background()

	newFeatureStore := func(name, featureRepoPath string) *feastdevv1.FeatureStore {
		return &feastdevv1.FeatureStore{
	...
	It("accepts a packaged repository with an absolute non-root path", func() {
		featureStore := newFeatureStore("packaged-valid", "/opt/feast/feature_repo")
		Expect(k8sClient.Create(ctx, featureStore)).To(Succeed())
		Expect(k8sClient.Delete(ctx, featureStore)).To(Succeed())
	})

	DescribeTable("rejects an invalid packaged repository path",
		func(name, featureRepoPath string) {
			err := k8sClient.Create(ctx, newFeatureStore(name, featureRepoPath))
			Expect(err).To(HaveOccurred())
			Expect(apierrors.IsInvalid(err)).To(BeTrue(), "expected invalid error, got %v", err)
			Expect(strings.ToLower(err.Error())).To(ContainSubstring("absolute, non-root path"))
		},
		Entry("relative", "packaged-relative", "opt/feast/feature_repo"),
		Entry("root", "packaged-root", "/"),
	)

	DescribeTable("rejects packaged together with another project directory mode",
		func(name string, conflictingMode func(*feastdevv1.FeastProjectDir)) {
			featureStore := newFeatureStore(name, "/opt/feast/feature_repo")
			conflictingMode(featureStore.Spec.FeastProjectDir)
		...
		Entry("init", "packaged-with-init", func(projectDir *feastdevv1.FeastProjectDir) {
			projectDir.Init = &feastdevv1.FeastInitOptions{}
		}),
		Entry("git", "packaged-with-git", func(projectDir *feastdevv1.FeastProjectDir) {
			projectDir.Git = &feastdevv1.GitCloneOptions{URL: "https://example.com/feature-repo.git"}
		}),
	)
})
